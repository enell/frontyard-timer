...
const jsWatcher = Bun.spawn(
	["bun", "build", "--target=browser", "./index.html", "--outdir=dist", "--watch"],
	{ stdout: "inherit", stderr: "inherit" },
);

// When either watcher exits (crash or signal), kill the sibling and propagate the exit code
const exitCode = await Promise.race([
	cssWatcher.exited.then((code) => { jsWatcher.kill(); return code; }),
	jsWatcher.exited.then((code) => { cssWatcher.kill(); return code; }),
]);
